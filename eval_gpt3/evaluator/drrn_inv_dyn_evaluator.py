...
class DrrnInvDynEvaluator(Trainer):
    def __init__(
        self,
        tb: logger.Logger,
        log: Callable[..., None],
    ...
    def update_envs(self, action_strs, action_ids, states, max_score: int,
        ...
        for i, (next_ob, next_reward, next_done, next_info, state, next_state, next_action_str) in enumerate(zip(next_obs, next_rewards, next_dones, next_infos, states, next_states, action_strs)):
            self.scores[i] = max(self.scores[i], next_info['score'])
            # Log
            self.log('(Env {}) Action_{}: {}'.format(
                i, self.steps, next_action_str), condition=(not self.env_finished[i]))
            self.log("(Env {}) Reward{}: {}, Score {}, Done {}".format(
                i, self.steps, next_reward, next_info['score'], next_done), condition=(not self.env_finished[i]))
            self.log('(Env {}) Obs{}: {} Inv: {} Desc: {}'.format(
                i, self.steps, clean(next_ob), clean(next_info['inv']),
                clean(next_info['look'])), condition=(not self.env_finished[i]))
    ...
    def train(self):
        ...
        self.env_finished = [False for _ in range(self.envs.num_envs)]
        self.scores = [0 for _ in range(self.envs.num_envs)]
        ...
        for step in range(1, self.max_steps + 1):
            ...
            # Log envs[0]
            for eid in range(self.envs.num_envs):
                s = ''
                for idx, (act, val) in enumerate(
                        sorted(zip(infos[eid]['valid'], action_qvals[eid]),
                            key=lambda x: x[1],
                            reverse=True), 1):
                    s += "{}){:.2f} {} ".format(idx, val.item(), act)
                self.log('(Env {}) Q-Values: {}'.format(eid, s), condition=(not self.env_finished[eid]))
            ...
            if self.end_step(step, start, max_score, action_qvals, max_eval):
                break
        return self.scores
